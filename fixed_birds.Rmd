...
for (i in knb_files) {
  # Clean the filename to create a valid object name
  birds <- i %>%
    str_remove("^52_pp52_") %>%  # Use ^ to anchor the prefix
    str_remove("\\.csv$")        # Escape the dot and anchor the suffix

  # Read the CSV and assign it to the cleaned name
  assign(birds, read_csv(here("knb-lter-cap_256_9", i)))
}
